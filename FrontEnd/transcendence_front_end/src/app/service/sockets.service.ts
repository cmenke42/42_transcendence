...
export class SocketsService {
  ...
  privateConnect(sender: string, receiver: string)
  {
    ...
    const private_socket = this.connectionAll(`wss://localhost:6010/ws/private_chat/${sender}/${receiver}/`);
  ...
  {
    return this.http.get(`https://localhost:6010/api/v1/get_messages/?sender=${sender}&receiver=${receiver}`);
  }

  messageRead(receiver:string, sender:string,) : Observable<any>
  {
    return this.http.put(`https://localhost:6010/api/v1/mark_message_as_read/`, { receiver, sender });
  }

  getUnreadMessageCounts(currentUser: string): Observable<any> {
    return this.http.get(`https://localhost:6010/api/v1/get_unread_message_counts/?current_user=${currentUser}`);
  }
  ...
  privateClose()
  {
    ...
    {
      const socket = this.connectionAll(`wss://localhost:6010/ws/online_status/`);
      socket.onopen = () =>
      {
        console.log('Websocket connected');
